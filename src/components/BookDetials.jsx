	...
	return (
		...
		<div className='flex flex-col max-w-screen-lg overflow-hidden bg-white border rounded shadow-sm lg:flex-row sm:mx-auto'>
		  ...
		  <div className=' p-8 bg-white lg:p-16 lg:pl-10 lg:w-1/2'>
			...
			{
			fold ? (<div>
				<p className=' text-gray-900'>Rating: {desc.substring(0,100)}..... </p>

				<span onClick={()=> setFold(!fold)} className='cursor-pointer text-blue-600 mb-5'>Read More</span>
			</div>
			) : (
				<div>
				<p className=' text-gray-900'>Rating: {desc}</p>

				<span onClick={()=> setFold(!fold)} className='cursor-pointer text-blue-600 mb-5'>Read Less</span>
				</div>
			)
			}
